  ...
  <layout-management>
    ...
    <div class="form-group">
      <div class="text--center wrap--content">
        <button class="ui-btn--main thick--big size--medium" @click="onSubmit">Update</button>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      news() {
        return this.$store.getters.getNew(this.id)
      },
      date() {
      ...
      news: {
        handler: function() {
          if(this.news) {
            ...
            this.formData.content = this.news.content
            this.formData.draft = this.news.draft
          }
        }
      }
    },
    mounted() {
      Vue.$service.news.fetchOneNew(this.id)
      .then((data) => {
        this.$store.dispatch('fetchOneNew', data)
      })
    },
    methods: {
      resetForm() {
      ...
      onSubmit() {
        ...
        Vue.$api.post('/news/update/' + this.id, {
          id: this.news.id,
